...
class DualLibrarySystem:
    ...
    def identify_from_behavioral_analysis(self, fingerprint_data: Dict) -> ModelIdentification:
        ...
        # More debug to see actual values
        if variance_profile:
            print(f"  - Variance profile sample: {variance_profile[:3]}")
        if restriction_sites:
            print(f"  - First site details: {restriction_sites[0] if restriction_sites else 'None'}")

        if not restriction_sites and not variance_profile:
            print(f"[DEBUG-IDENTIFY] RETURNING 0% - No data!")
            return ModelIdentification(
                identified_family=None,
                confidence=0.0,
    ...
    def _compute_topology_similarity(self, sites1, sites2, profile1, profile2, layer_count1=None, layer_count2=None):
        ...
        """
        # Allow matching based on profiles even without explicit restriction sites
        has_sites1 = sites1 and len(sites1) > 0
        has_sites2 = sites2 and len(sites2) > 0
        has_profile1 = profile1 and len(profile1) > 0
        has_profile2 = profile2 and len(profile2) > 0

        if not has_sites1 and not has_profile1:
            print(f"[DEBUG-TOPOLOGY] No data for fingerprint1: sites={sites1}, profile={profile1}")
            return 0.0
        if not has_sites2 and not has_profile2:
            print(f"[DEBUG-TOPOLOGY] No data for fingerprint2: sites={sites2}, profile={profile2}")
            return 0.0
        ...
        pattern1 = extract_delta_pattern(sites1, layer_count1) if has_sites1 else []
        pattern2 = extract_delta_pattern(sites2, layer_count2) if has_sites2 else []

        # If no sites but we have profiles, create patterns from variance changes
        if not pattern1 and profile1 and len(profile1) > 1:
            pattern1 = []
            for i in range(1, len(profile1)):
                delta = profile1[i] - profile1[i-1]
                pattern1.append({
                    "delta": abs(delta),
                    "direction": 1 if delta > 0 else -1 if delta < 0 else 0,
                    "percent": abs(delta / profile1[i-1] * 100) if profile1[i-1] != 0 else 0,
                    "relative_pos": i / len(profile1)
                })

        if not pattern2 and profile2 and len(profile2) > 1:
            pattern2 = []
            for i in range(1, len(profile2)):
                delta = profile2[i] - profile2[i-1]
                pattern2.append({
                    "delta": abs(delta),
                    "direction": 1 if delta > 0 else -1 if delta < 0 else 0,
                    "percent": abs(delta / profile2[i-1] * 100) if profile2[i-1] != 0 else 0,
                    "relative_pos": i / len(profile2)
                })
        ...
        if pattern1 and pattern2 and len(pattern1) > 0 and len(pattern2) > 0:
        ...
        elif profile1 and profile2:
            # Only profiles available - compare profiles directly
            # 70% profile similarity, 30% delta patterns from profiles
            return 0.7 * profile_similarity + 0.3 * delta_similarity
        else:
            # Fallback to whatever we have
            return max(delta_similarity, position_similarity, profile_similarity)
